...
const reducer = combineReducers({intl: intlReducer, scratchPaint: ScratchPaintReducer});
const store = createStore(reducer, {
  intl: {defaultLocale: 'en', locale: 'en', messages: paintMessages.en.messages}
});
...
function App() {
  ...
  const openFile = e => {
    ...
    reader.onload = () => {
      if (typeof reader.result === 'string') { setImage(reader.result); setName(file.name); }
    };
  ...
  return <div className="app">
    ...
    <main className="editor">
      <PaintEditor
        image={image} imageFormat="svg" imageId={imageId} name={name}
        rotationCenterX={240} rotationCenterY={180}
        onUpdateName={setName}
        onUpdateImage={newImage => { if (typeof newImage === 'string') setImage(newImage); }}
      />
    </main>
  </div>;
}
...
createRoot(document.getElementById('root')).render(
  <Provider store={store}><IntlProvider><App /></IntlProvider></Provider>
);
